...
router.get('/', function(req, res, next) {
  querySelect(`select  
  (select count(id) from empresas) as empresas_total, (select count(id) from users) as usuarios_total
  `)
  .then(resultCount => {
    ...
    .then(resultRubro => {
      ...
      .then(resultCategoria => {
        ...
        .then(resultUltimasVentas => {
          querySelect(`select users.id, users.nombre, users.apellido, sol.contador
          from users
          inner join (
          ...
          .then(resultStatsVentas => {
            res.json({resultCount: resultCount, resultRubro: resultRubro, resultCategoria: resultCategoria, resultUltimasVentas: resultUltimasVentas, resultStatsVentas: resultStatsVentas})
          })
        })
